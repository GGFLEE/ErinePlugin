...
class Ernie:
    ...
    def ask(self, question: str, sessionId='', parentChatId='0') -> dict:
        if  sessionId=='':
            self.sessionId = self.newConversation(question)
        else:
            if parentChatId=='0':
                raise Exception('parentChatId未传入')
        ...
        self.parentChatId=botChatId
        return {
            'answer': fullAnswer,
            'imageurls': re.findall(imagePattern, data['tokens_all']),
            'botChatId': botChatId,
            'sessionId':self.sessionId
        }
